...
logging.basicConfig(filename='/home/satark_web/logs/ec_ws_log.log', level=logging.DEBUG)
...
def main(ncfilename,out_dir):
	...
	# yoda =  5 * (ncf.variables['t2m'][4] -  ncf.variables['d2m'][4])
	LtM = (lats >= 4) & (lats <= 45)
	LoM = (lons >= 45) & (lons <= 100)

	# print ( ncf.variables['t2m'][4][yoda>100]-273.16 )
	# print ( ncf.variables['d2m'][4][yoda>100]-273.16 )
	# print(yoda.shape)
	# return
	...
	for tid, hr  in np.ndenumerate(times[:]):
		...
		if hr%24==0 and tid!=0:
			...
			wsT = np.average(ws[tid-3:tid],axis=0)
			

			ax = plt.axes(projection = ccrs.PlateCarree())

			croppedWS = wsT[np.ix_(LtM,LoM)]
			print(croppedWS.min())

			# lvls = np.array([-10,-5,0,5,10,12,15,17,20,22,25,27,30,35,40,45,50])
			lvls = np.arange(5,115,5)
			

			cf = plt.contourf(lons[LoM],lats[LtM],croppedWS,cmap='cividis_r',levels=lvls,extend='both')
			...
			geojsonFname = f'ws.avg.at_{analysis_time}.d_{int(del_hr/24):02}.geojson'
			...
			cbarFname = f'cbar.ws.avg.ad_{analysis_time}.d_{int(del_hr/24):02}.svg'
			...
			finfo.append({
				...
				'cbar':cbarFname,
				'day':int(del_hr/24),
				'fdate':dates[tid].strftime('%Y-%m-%d') 
			})
	...
	cur = __db__.cursor()
	query = "UPDATE forecast_state SET ec_ws_dly_update = '"+str(analysis_date)+"'";
...
if __name__ == '__main__':
	...
	#sys.argv.append('/home/nazmul/www/ifas/forecast_data/ecmwf_nc/20190620R1D.nc')
	#sys.argv.append('/home/nazmul/www/osdma/DATA/forecast_data/ecmwf/ws/')
