...
logging.basicConfig(filename='/home/satark_web/logs/ec_tmin_log.log', level=logging.DEBUG)
...
def main(ncfilename,out_dir):
	...
	for tid, hr  in np.ndenumerate(times[:]):
		...
		if hr%24==0 and tid!=0:
			...
			tempT = np.amin(temp[tid-3:tid+1],axis=0)
			...
			geojsonFname = f'temp.min.at_{analysis_time}.d_{int(del_hr/24):02}.geojson'
			...
			cbarFname = f'cbar.temp.min.ad_{analysis_time}.d_{int(del_hr/24):02}.svg'
	...
	cur = __db__.cursor()
	query = "UPDATE forecast_state SET ec_tmin_dly_update = '"+str(analysis_date)+"'";
...
if __name__ == '__main__':
	...
	# sys.argv.append('/home/nazmul/www/osdma/DATA/forecast_data/ecmwf/tmin/')
